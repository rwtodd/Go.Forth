...
func comparisonWordsInit(vm *VM) {
	vm.Define(&NativeWord{name: "=", run: equal, immediate: false})
	vm.Define(&NativeWord{name: "<", run: lessThan, immediate: false})
	vm.Define(&NativeWord{name: ">", run: greaterThan, immediate: false})
	vm.Define(&NativeWord{name: "<=", run: lessThanOrEqual, immediate: false})
	vm.Define(&NativeWord{name: ">=", run: greaterThanOrEqual, immediate: false})
	vm.Define(&NativeWord{name: "<>", run: notEqual, immediate: false})
	vm.Define(&NativeWord{name: "0=", run: zeroEqual, immediate: false})
	vm.Define(&NativeWord{name: "0<", run: zeroLessThan, immediate: false})
	vm.Define(&NativeWord{name: "0>", run: zeroGreaterThan, immediate: false})
	vm.Define(&NativeWord{name: "and", run: andOp, immediate: false})
	vm.Define(&NativeWord{name: "or", run: orOp, immediate: false})
	vm.Define(&NativeWord{name: "xor", run: xorOp, immediate: false})
	vm.Define(&NativeWord{name: "invert", run: invertOp, immediate: false})
	vm.Define(&NativeWord{name: "true", run: trueConst, immediate: false})
	vm.Define(&NativeWord{name: "false", run: falseConst, immediate: false})
}
