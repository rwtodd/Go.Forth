...
func TestVMake(t *testing.T) {
	// 1. >>@i
	tstRunForth(t, "VMakeInts", `<< 10 20 30 >>@i dup @len swap 1 @`, 3, 20)

	// 2. >>@f
	tstRunForth(t, "VMakeFloats", `<< 1.5 2.5 >>@f dup @len swap 0 @`, 2, 1.5)

	// 3. >>@s
	tstRunForth(t, "VMakeStrings", `<< " hello" " world" >>@s dup @len swap 0 @`, 2, "hello")
}
...
func TestVPushErrors(t *testing.T) {
	...
	// Test vmake errors
	tstRunForthErrIgnoreStack(t, "VMakeIntError", `<< " s" >>@i`, ErrArgument)
}

func TestVMakeMixed(t *testing.T) {
	// 4. >> (any)
	tstRunForth(t, "VMakeAny", `<< 1 " hi" 3.14 >> dup @len swap dup 1 @ swap 0 @`, 3, "hi", 1)
