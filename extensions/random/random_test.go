...
func TestRandomExtension(t *testing.T) {
	vm := forth.NewVM()
	err := vm.Run(strings.NewReader(`<<" random ">> <activate-extensions>`), io.Discard)
	if err != nil {
		t.Fatalf("Failed to activate random extension: %v", err)
	}

	t.Run("randint", func(t *testing.T) {
		err := vm.Run(strings.NewReader("10 4 randint"), io.Discard)
		if err != nil {
	...
	t.Run("randfloat", func(t *testing.T) {
		err := vm.Run(strings.NewReader("randfloat"), io.Discard)
		if err != nil {
	...
	t.Run("@seed", func(t *testing.T) {
		err := vm.Run(strings.NewReader("42 randseed!"), io.Discard)
		if err != nil {
			t.Fatalf("@seed failed: %v", err)
		}

		err = vm.Run(strings.NewReader("100 0 randint"), io.Discard)
		if err != nil {
			t.Fatalf("randint after seed failed: %v", err)
		}
		val1, _ := vm.Pop()

		err = vm.Run(strings.NewReader("42 randseed! 100 0 randint"), io.Discard)
		if err != nil {
	...
	t.Run("@select", func(t *testing.T) {
		err := vm.Run(strings.NewReader(`<< 10 20 30 >> <ints> @select`), io.Discard)
		if err != nil {
	...
	t.Run("@shuffle", func(t *testing.T) {
		err := vm.Run(strings.NewReader(`<< 10 20 30 >> <ints> constant y  y @shuffle`), io.Discard)
		if err != nil {
			t.Fatalf("@shuffle failed: %v", err)
		}
		// Since it's shuffled, elements should still be exactly 10, 20, 30.
		err = vm.Run(strings.NewReader(`y 0 @ y 1 @ y 2 @ + +`), io.Discard)
		if err != nil {
